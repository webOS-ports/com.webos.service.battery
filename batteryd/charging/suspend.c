...
#define LOG_DOMAIN "BATTERYD-SUSPEND: "


#define kBatteryCheckReasonSysfs "/sys/power/batterycheck_wakeup"
#define kChargerWakeupSourcesSysfs      "/sys/power/wakeup_event_list"
...
_ParseWakeupSources(int resumeType)
{
    ...
    {
        BATTERYDLOG(LOG_INFO, "Wakeup Source: [ 0.0 ] %s () %s (0)", resume_type_descriptions[resumeType], resume_type_descriptions[resumeType]);
        goto done;
    }

    /* kBatteryCheckReasonSysfs doesn't exist on all systems, but
...
cleanup:
    BATTERYDLOG(LOG_DEBUG, "batteryd awoke with batterycheck %s.", batterycheck_reason);

    if (wakeup_source_list != NULL) {
        for (i=0; wakeup_source_list[i] != NULL ;i++) {
            BATTERYDLOG(LOG_INFO, "Wakeup Source: %s", wakeup_source_list[i]);
        }
